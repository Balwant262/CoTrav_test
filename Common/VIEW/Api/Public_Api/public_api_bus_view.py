...
def create_bus_booking(request):
    if 'AUTHORIZATION' in request.headers:
        ...
        if not all (k in received_json_data for k in ("travel_request_number","from_city", "to_city", "bus_type","preferred_bus","preferred_board_point",
            "preferred_drop_point","journey_from_date","journey_from_time","journey_to_date","journey_to_time","reason_booking","project_city","ProjectCode","Passengers")):
        ...
        if received_json_data:
            ...
            preferred_board_point = received_json_data['preferred_board_point']
            preferred_drop_point = received_json_data['preferred_drop_point']
            preferred_bus = received_json_data['preferred_bus']
            bus_type = received_json_data['bus_type']
                ...
                if user_token[0] == 'Token':
                    ...
                    if user:
                        ...
                        if ('ProjectCode' in received_json_data) :
                            for ass in received_json_data['ProjectCode']:
                                ...
                                if ass['project_code'] and ass['valid_from_date'] and ass['valid_to_date']:
                                    ...
                                    data = {'success': 0, 'message': "Please Provide Project Code Service Date"}
                                    return JsonResponse(data)
                        else:
                            assessment_code = 0
                        ...
                        try:
                            ...
                            cursor.callproc('addBusBooking', [user_type, user_id, corporate_id, spoc_id, group_id, subgroup_id, from_city, to_city, bus_type, bus_type, bus_type, booking_datetime,
                                             ...
                                             journey_datetime_to, '@last_booking_id', '@booking_reference_no', bta_code_travel_req_no, preferred_board_point, preferred_drop_point])
                            ...
                            if booking_id:
                                ...
                                thread = Thread(target=add_booking_email.send_taxi_email,
                                                args=(emp, approvers, "Bus"))
                                thread.start()
                                thread = Thread(target=add_booking_email.send_taxi_msg, args=(emp, approvers, "Bus"))
                                thread.start()

                            data = {'success': 1, 'message': "Bus Booking Added Successfully..! Your Booking ID is : " + str(booking_reference_no), 'booking_id': booking_reference_no}
                            return JsonResponse(data)
        ...
        if received_json_data:
            ...
            if user_token[0] == 'Token':
                ...
                if user:
                    ...
                    corporate_id = user[0]['corporate_id']
                    cursor2 = connection.cursor()
                    cursor2.callproc('getPublicApiCorporateBookings', [corporate_id,service_type,from_date,to_date,booking_id,2])
                    ...
                    for i, e in enumerate(data):
                        ...
                        try:
                            ...
                            rq_details['preferred_bus'] = e['preferred_bus']
                            rq_details['boarding_point'] = e['boarding_point']
                            rq_details['droping_point'] = e['droping_point']
                            rq_details['no_of_seats'] = e['no_of_seats']
                            rq_details['bus_type'] = e['bus_type']
                            ...
                            assign_details['bus_type'] = e['bus_type']
                            assign_details['boarding_date'] = e['boarding_date']
                            assign_details['boarding_time'] = e['boarding_time']
                            assign_details['pnr_no'] = e['pnr_no']
                            assign_details['boarding_point'] = e['boarding_point']
                            assign_details['droping_point'] = e['droping_point']
